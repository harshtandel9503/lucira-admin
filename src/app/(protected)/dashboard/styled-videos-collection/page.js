...
import { Search, Plus, Trash2, Save, MoveUp, MoveDown, Video, Package, X, Loader2, Layers } from 'lucide-react';
import Image from 'next/image';
import { toast } from 'react-toastify';
import SafeVideo from '../../../../components/common/SafeVideo';

export default function StyledVideosCollectionDashboard() {
  ...
  const fetchVideos = async () => {
    try {
      const res = await fetch('/api/styled-videos-collection');
  ...
  const addVideo = () => setVideos([...videos, { video: '', collectionHandle: '', products: [], totalPrice: '₹0' }]);
  ...
  const updateVideoUrl = (index, url) => {
    const nv = [...videos];
    nv[index] = { ...nv[index], video: url };
    setVideos(nv);
  };

  const updateCollectionHandle = (index, handle) => {
    const nv = [...videos];
    nv[index] = { ...nv[index], collectionHandle: handle };
    setVideos(nv);
  };
  ...
  return (
    <div className='max-w-7xl mx-auto py-10 px-8'>
      <div className='flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6'>
        <div>
          <h1 className='text-zinc-900 text-[24px] font-bold font-figtree tracking-[0.1px]'>Styled Video Collection Specific</h1>
          <p className='text-zinc-500 mt-1' style={{ marginTop: '2px', fontSize: '16px', color: '#000' }}>Manage collection-specific videos and tagged products.</p>
        </div>
                ...
                <div className='flex-1 space-y-8'>
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                    <div className='space-y-3'>
                      <label className='text-[10px] font-bold uppercase tracking-widest text-zinc-400 flex items-center gap-2'><Video size={12} /> SHOPIFY VIDEO URL</label>
                      <input value={video.video || ''} onChange={e => updateVideoUrl(vIndex, e.target.value)} className='w-full px-5 py-3.5 bg-zinc-50 border border-zinc-100 rounded-[8px] text-sm focus:outline-none focus:ring-2 focus:ring-black' placeholder='Paste Shopify CDN URL here...' />
                    </div>
                    <div className='space-y-3'>
                      <label className='text-[10px] font-bold uppercase tracking-widest text-zinc-400 flex items-center gap-2'><Layers size={12} /> COLLECTION HANDLE</label>
                      <input value={video.collectionHandle || ''} onChange={e => updateCollectionHandle(vIndex, e.target.value)} className='w-full px-5 py-3.5 bg-zinc-50 border border-zinc-100 rounded-[8px] text-sm focus:outline-none focus:ring-2 focus:ring-black' placeholder='e.g. hexa, rings, etc.' />
                    </div>
                  </div>
                      ...
                      {video.products?.map((p, pIndex) => (
                        <div key={pIndex} className='bg-white p-3 rounded-[8px] border border-zinc-100 flex gap-4 relative group hover:shadow-md transition-all'>
                          ...
                          <div className='absolute right-2 top-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity'>
                            ...
                            <button onClick={() => { 
  const nv = [...videos]; 
  const video = { ...nv[vIndex], products: nv[vIndex].products.filter((_, i) => i !== pIndex) };
  const total = video.products.reduce((acc, p) => acc + (parseInt(p.price.replace(/[^\d]/g, '')) || 0), 0);
  video.totalPrice = '₹' + new Intl.NumberFormat('en-IN', { maximumFractionDigits: 0 }).format(Math.round(total));
  nv[vIndex] = video;
  setVideos(nv); 
}} className='p-1.5 hover:bg-rose-50 text-rose-300 hover:text-rose-500 rounded-[8px]'><Trash2 size={14} /></button>
                          </div>
                        </div>
                      ))}
